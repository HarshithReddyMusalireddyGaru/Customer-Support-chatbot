...
def db_search(state):
    ...
    QUERY = """
        Now, analyze the following code files and database files attached. I am encountering an issue where the chatbot is providing responses where it is directly mentioning about the answer not being in the context, where it is supposed to fulfil the requirements mentioned in the proposal document where in it is supposed to act as a conversational customer service chatbot which functions unlike the regular rule-based chatbots, responds in a friendly-conversational humanly like manner, is powered by Llama3 LLM, Agentic RAG with two agents faq_agent, and sql agent (files attached) and using an faqs file and ecommerce database file respectively, which is attached, has a UI built using streamlit. The problem is that the chatbot responses are lacking continuity and flow of the conversation, I need you to modify the code in such a manner where it should be able to conversate like a regular LLM without going out of the context, and shouldn't mention about any files by which it is backed up. Am I to modify the query in the code or anything else. If I am to modify the query in the code, act as a prompt engineer and write an effective query in accordance with the code files and project's purpose to leverage the best.
